...
function DataLoader:__init(opt)
  ...
  self.info = utils.read_json('config.json')
  ...
  self.h5_file = hdf5.open('label.h5', 'r')
  self.h5_file2 = hdf5.open('result.h5','r')
...
function DataLoader:getBatch(opt)
  ...
  local batch_size = utils.getopt(opt, 'batch_size', 90) -- how many images get returned at one time (to go through CNN)
  ...
  local img_batch_raw = torch.DoubleTensor(batch_size,4096)
  ...
  for i=1,batch_size do
    ...
    -- img_batch_raw[i] = img
    local img = self.h5_file2:read('/data'):partial({ix,ix},{1,4096})
    img_batch_raw[i] =img
    -- print(img)
    -- print(img)
