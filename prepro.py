...
def build_vocab(tsfile):
  ...
  for i,line in enumerate(tsfile):
    ...
    for w in line.split(','):
      # if w[0] >= 'a' and w[0] <= 'z':
        # w = str(w).lower().translate(None, string.punctuation).strip()
        # ordered_captions[i].append(w)
  ...
  # vocab = [w for w,n in counts.iteritems() if n>=3]
...
def encode_captions(imgs):
  ...
  for i,img in enumerate(imgs):
    ...
    label_length[i] = min(max_length, len(img)) # record the length of this sequence
    # caption_counter += 1
    f = 0
    for k,w in enumerate(img):
      if k < max_length:
        Li[0,f] = w
        f = f+1
        # else:
        #   label_length[i] = label_length[i] - 1
  ...
  # print L
  # print L.shape,N
  # assert L.shape[0] == N, 'lengths don\'t match? that\'s weird'
  # print sum(label_length ==0)
...
def main():
  ...
  # create the vocab
  ordered_captions = build_vocab(tsfile)
 # itow = {i+1:w for i,w in enumerate(vocab)} # a 1-indexed vocab translation table
 # wtoi = {w:i+1 for i,w in enumerate(vocab)} # inverse table

  # assign the splits
  splits = [0 for x in range(len(ordered_captions))]
  assign_splits(splits)
  
  # encode captions in large arrays, ready to ship to hdf5 file
  L, label_start_ix, label_end_ix, label_length = encode_captions(ordered_captions)

  # create output h5 file
  N = len(ordered_captions)
  f = h5py.File('label.h5', "w")
  ...
  json.dump(out, open('config.json', 'w'))
